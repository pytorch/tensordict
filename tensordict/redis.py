...
class RedisTensorDict(TensorDictBase):
    ...
    async def _aset_tensor(
        self, key_path: str, tensor: torch.Tensor
    ):
    ...
    async def _aset_non_tensor(self, key_path: str, value: Any):
        ...
        data = serialized if isinstance(serialized, bytes) else serialized.encode("utf-8")
        pipe = self._client.pipeline()
        pipe.set(self._data_key(key_path), data)
        pipe.hset(self._meta_key(key_path), mapping=meta)
        pipe.sadd(self._keys_registry_key, key_path)
    ...
    def close(self):
        ...
        if hasattr(self, "_owns_loop") and self._owns_loop:
            if hasattr(self, "_client") and hasattr(self, "_loop") and self._loop.is_running():
                try:
    ...
    def _memmap_(self, *, prefix, copy_existing, executor, futures, inplace,
                 like, share_non_tensor, existsok, robust_key):
        raise RuntimeError(
    ...
    def _add_batch_dim(self, *, in_dim, vmap_level):
        raise RuntimeError(
            f"{type(self).__name__} cannot be used with vmap."
        )
